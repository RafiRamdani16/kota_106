...
class EditOvertimeScreen extends GetView<OvertimeController> {
  ...
  Widget build(BuildContext context) {
    controller.overtimeSelectedDate.value =
        DateTime.parse(overtimeModel.overtimeDate);
    // DateTime formated = DateTime.parse(overtimeModel.overtimeStartTime);
   
    // DateTime format = DateTime.parse(formatedStartTIme);
    controller.overtimeStartTime.value =
        TimeOfDay.fromDateTime(DateFormat("HH:mm").parse("${overtimeModel.overtimeStartTime}"));
     controller.overtimeEndTime.value = TimeOfDay.fromDateTime(
        DateFormat("HH:mm").parse("${overtimeModel.overtimeEndTime}"));
    controller.overtimeDescription.text = overtimeModel.overtimeDescription;
    return Scaffold(
      appBar: AppBar(
        elevation: 0,
                              ...
                              Obx((() {
                                return Container(
                                    ...
                                    child: ElevatedButton(
                                      ...
                                      onPressed: () {
                                        controller.editOvertimeDatePicker(context);
                                      },
                                      child: Text(
